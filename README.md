...
實驗結果:
┌─────────────────────────────┬──────────────┬──────────────┐
│ 實驗配置                     │ Recall@10    │ MRR@10       │
├─────────────────────────────┼──────────────┼──────────────┤
│ Retriever Only (Top 3)      │ X.XXXX       │ X.XXXX       │
│ Retriever + Reranker (Top 3)│ X.XXXX       │ X.XXXX       │
│ Retriever Only (Top 5)      │ X.XXXX       │ X.XXXX       │
└─────────────────────────────┴──────────────┴──────────────┘

觀察與結論:
1. Reranker 的影響:
   - MRR 提升: +X.XX% (實驗2 vs 實驗1)
   - [分析 Reranker 是否顯著提升排序品質]

2. 增加輸入筆數的效果:
   - MRR 變化: ±X.XX% (實驗3 vs 實驗1)
   - [分析增加候選數量是否有幫助]

3. 能否彌補 Reranker:
   - 實驗3 vs 實驗2 的 MRR 差距: X.XXXX
   - [結論: 能/不能透過增加輸入筆數彌補沒有 Reranker]
```
